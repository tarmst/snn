...
from mnist_sg_cnn.base_model import LIFLayer
...
class SSDClassificationHead(SSDScoringHead):
	def __init__(self, in_channels: List[int], num_anchors: List[int], num_classes: int):
		...
		for channels, anchors in zip(in_channels, num_anchors):
			base_layer = nn.Conv2d(channels, num_classes * anchors, kernel_size=3, bias=False)
...
class SSDRegressionHead(SSDScoringHead):
	def __init__(self, in_channels: List[int], num_anchors: List[int]):
		...
		for channels, anchors in zip(in_channels, num_anchors):
			base_layer = nn.Conv2d(channels, 4 * anchors, kernel_size=3, bias=False)
			layer = LIFLayer(base_layer,
							 alpha=.9,
							 beta=.85,
							 alpharp=.65,
